...
ref <- args[6]    #Ref sequence.
Reference <- read.fasta(ref)
...
#' There is somethign raising an error here sometimes  
...
#this is the major function of this script
...
printCoverageStats <- function(bamFile, sampleName, outDir = getwd(), ...) {
  ...
  #end(indels[which(names(indels) == 'I')]) <- start(indels[which(names(indels) == 'I')]) # this was from original fine but I think I need the +1
  seqinfo(indels) <- seqinfo(aln)
  
  del <- indels[which(is.element(names(indels), c('D', 'N')))]
  ins <- indels[which(names(indels) == 'I')]

insertedSequencesFile <- file.path(outDir, paste0(sampleName,'.insertedSequences.tsv')) #past0 function links sampleName w/ ,insertedSequences.tsv. Simliar to paste() but there you have to delclare no space inserted
...
countEventsAtCutSite <- function(seqName, cutStart, cutEnd, bamFile, readsWithInDels, extend = 3) {
  ...
  stats <- data.frame('indel' = length(indelsAtCutSites), 
                      'del' = sum(is.element(names(indelsAtCutSites), c('D', 'N'))),
...
deletions <- indels[ is.element(type, c('D', 'N')), c('seqname', 'start', 'end', 'ID', 'ReadSupport')]
